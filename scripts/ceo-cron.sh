...
if [ "$RUNNER" = "ollama-agent" ]; then
  ...
  # The bridge records MCP failures too (#271), skipping read-only MCP probe misses (#457)
  # while still recording transport faults (crashes, timeouts) even on read-only tools (#475),
  # so any mutating MCP failure, transport fault, or unannotated MCP error surfaces here.
