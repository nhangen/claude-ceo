...
ceo_setup_write_next_steps() {
  ...
  local yq_hint="${1:-sudo snap install yq  (or: brew install yq)}"
  {
    ...
    echo "     Then type:  /ceo"
    echo "  9. Smoke test:  ceo test"
    echo " 10. Install the ceo-schedulerd keep-alive agent from lib/scheduler/deploy/"
    echo "     (it is the only scheduler; 'ceo doctor' must show 'ceo-schedulerd alive')"
    echo ""
