...
- **`gh`/`jq` per-repo failure (auth expiry, rate-limit, network, deleted/renamed remote)**: status `clear`, `last_error: gh_failed:<slug>` (etc.), cursor does NOT advance, `consec_gh_errors` increments. The next tick retries the same merge window. Per `safety-invariant-scope`, `gh` errors and "no merges" are distinguished structurally — failures never collapse to a clean tick. **Bounded retry (#149):** a *permanently* failing repo would otherwise wedge the cursor forever; after `MAX_RETRIES` (3) consecutive gh-error ticks the cursor advances anyway and a self-deduping `<!-- triage-autopilot:giveup-gh:<date> -->` line naming the `last_error` is appended to `inbox.md`. Any clean-fetch tick resets `consec_gh_errors` to 0, so transient/flaky errors never accumulate to give-up.
