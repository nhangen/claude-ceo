...
inputs: [pr_data, pending_count, today_log, yesterday_log, daily_note, active_domains, pending_ask, yesterday_merged, ledger_recent]
---
...
2. **Priorities (ranked by REAL signal).** Rank today's work. Never rank by age alone — an old PR is not urgent because it is old. Use `Daily note Top 3` as the primary key, then review-blocking state (a PR where someone is waiting on you outranks one where you are waiting on them). Show the top 3-5 with a one-clause justification each ("Top 3 today", "review requested 3d ago").
