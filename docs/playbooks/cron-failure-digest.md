...
1. ONE `run_shell`. Run only this command — it prints the output path, creates the directory, and emits all relevant log content with this digest's own lines removed:
   The completion-log glob is deliberately host-agnostic, unlike `ceo doctor`'s cross-check: a peer's failure line is worth surfacing in a digest even though it must not be cross-checked against this host's artifact paths. Note the tail slices the concatenation, so ordering is by filename, not time.

   echo "OUTPATH=reports/cron-failures/$(date +%F)-$(hostname -s).md"; mkdir -p reports/cron-failures; { cat log/cron-runs.log log/cron-runs-*.log 2>/dev/null | tail -n 200; echo "===SKIPS==="; tail -n 200 log/cron-skips.log 2>/dev/null; } | grep -v cron-failure-digest
