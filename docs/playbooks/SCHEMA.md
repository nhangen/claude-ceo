...
| `model` | depends on `runner` | string | Claude model (e.g. `haiku`, `sonnet`) for `runner: claude`; Ollama tag for `runner: ollama` / `runner: ollama-think`. For `runner: script` it is optional and exported as `CEO_MODEL` so a script that itself drives an LLM (e.g. `ticket-triage-autopilot`) pins and reports the model in its Discord embed; omit it for pure-shell scripts. Ignored for `runner: skill`. |
