...
| `preflight` | recommended | function name | Bash function defined in `scripts/ceo-cron.sh` that gates whether the playbook runs (e.g. `has_unchecked_inbox`, `has_pending_items`, `none`). Omitting the field means `none` — the playbook runs ungated. Naming a `preflight_<name>()` that does not exist is different, and fatal: the run records a failure and exits 78 without dispatching, rather than running without the gate it declared. `ceo-scan.test.sh` lints this against the shipped playbooks, including `disabled` and `draft` ones, since a manual run of those reaches the same branch. |
...
`--dry-run` is a preview mode, orthogonal to run-mode. It runs every **read-only** phase (gather, the PLAN call, a read-tier model call) but mutates **no CEO state**: the EXECUTE phase is skipped, `runner: script` / `runner: skill` are not executed, and nothing is written to the approvals queue, Discord, the report intake, the host inbox, the synced daily log (`CEO/log/<TODAY>.md`), `.last-run`, `.last-scan`, the fail-counter, or `cron-runs-<host>.log`. What *would* happen is written to a preview file at `$HOME/.ceo/state/preview/<trigger>-<TODAY>.md`.

That preview is **host-local** by location, not by rule: since #394 it lives under `$HOME/.ceo/state/`, outside the synced vault, along with the per-trigger fail counter, the cooldown stamp, and the scan marker. It used to sit in `CEO/log/preview/` and depend on each host having copied `syncthing/shared.stignore` into its vault root — protection nothing verifies, and which both swarm hosts were found missing an entry from on 2026-09-09. The stignore patterns remain as coverage for the files an upgrading host still has in its vault before its first run migrates them. The rest of `CEO/log/` *is* synced — the daily log and the operational diagnostic journals — which is why the daily-log header write is also skipped in dry-run. Since #399 those journals are keyed by host (`cron-skips-<host>.log`, `cron-stdout-<host>.log`, `cron-stderr-<host>.log`) and stay synced on purpose: they are journals rather than decision state, a peer's ERROR line is worth reading, `cron-failure-digest` reaches them by a path relative to the vault and could not follow them out of it, and one writer per file means syncing costs no conflicts. Every host sharing one copy of each is what produced the `.sync-conflict` copies #399 was filed for. A dry-run still appends clearly-labelled diagnostic lines to its host's skips journal (e.g. the `--scheduled` WARN below); that journal is the dispatcher's operational debug channel, not CEO decision-state.

`--dry-run` bypasses the cooldown so it can be run iteratively, and is allowed under `--scheduled` (with a WARN to `cron-skips-<host>.log`) so a daemon can smoke-test without acting. Non-guarantee: read-only external calls still run and still cost tokens — `--dry-run` skips effects, not reads.
...
**Output:** `$HOME/.ceo/state/preview/test-all/<TODAY>.md` — host-local (under the same out-of-vault preview tree as a single dry-run's preview), so a fleet smoke-test stays on the host that ran it. The report has a per-playbook result table (`would run` / `skip: no work` / `FAILED`) plus each playbook's individual preview inline.
